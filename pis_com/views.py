...
class CustomPasswordResetView(PasswordResetView):
    template_name = 'registration/password_reset_form.html'  # Указываем кастомный шаблон для страницы сброса пароля
    email_template_name = 'registration/password_reset_email.html'  # Шаблон письма с ссылкой на сброс пароля
    subject_template_name = 'registration/password_reset_subject.txt'  # Тема письма
    success_url = reverse_lazy('password_reset_done')  # Указываем URL, куда перенаправить после успешного сброса пароля

    def form_valid(self, form):
        # Получаем первого пользователя из генератора
        users = form.get_users(form.cleaned_data["email"])
        user = next(users, None)  # Если генератор пустой, вернется None

        if user:
            # Вызываем кастомную функцию отправки письма
            send_password_reset_email(user)

        return super().form_valid(form)

    # Переопределяем метод отправки email, чтобы избежать стандартной отправки
    # def send_mail(self, subject_template_name, email_template_name, context, from_email, to_email, html_email_template_name=None):
    #     # Отключаем стандартную отправку письма
    #     pass
...
class RegisterView(FormView):
    ...
    def form_valid(self, form):
        ...
        user.email = self.request.POST.get('email')  # Получение email из формы
        user.save()
        ...
        # Отправка приветственного письма на email
        send_mail(
            'Добро пожаловать в систему!',
            'Спасибо за регистрацию! Ваш аккаунт создан.',
            'noreply@yourdomain.com',  # Укажите реальный email отправителя
            [user.email],
            fail_silently=False,
        )
    ...
    def get_context_data(self, **kwargs):
        ...
        if self.request.POST:
            context.update({
                ...
                'email': self.request.POST.get('email'),  # Добавлено поле email
